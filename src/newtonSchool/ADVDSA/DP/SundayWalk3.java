...
public class SundayWalk3 {
    public static double sundayWalkSmall(double[] walk, int m, int main, double p, double q, double ans){
        for(int i=2; i<=3; i++) {
            int s = m - i;
            if (s == 0) {
                if (i == 2) {
                ...
                if (walk[s] != 0 && walk[s]!= -1) {
                    ...
                    double ans1 = 0;
                    double jj = sundayWalkSmall( walk, s, main, p, q, ans1);
                    walk[s] = jj;
                    if(jj >-1) {
                        if (i == 2) {
                            ans += walk[s] * p;
    ...
    public static double sundayWalk(double[] walk, int m, int main, double p, double q, double ans){
        for(int i=2; i<=3; i++) {
            ...
            if(s != -1) {
                int sd = main - s;
                if (walk[sd] == 0) {
                    double jj = sundayWalkSmall(walk, sd, main, p, q, 0);
                    walk[sd] = jj;
                }
            }
            if (s == 0) {
